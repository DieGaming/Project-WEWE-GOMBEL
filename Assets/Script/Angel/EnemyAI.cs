...
public class EnemyAI : MonoBehaviour
{
    ...
    private SanityManager sanityManager; // Reference to SanityManager script
    ...
    private void Start()
    {
        ...
        if (playerObject != null)
        {
            ...
            {
                sanityManager.SetEnemy(transform); // Set this enemy's transform in the SanityManager
            }
        }
        ...
        switch (currentState)
        {
            ...
            case EnemyState.Chase:
                ...
                if (Vector3.Distance(transform.position, player.position) > sightDistance)
                {
                    ...
                    if (sanityManager != null)
                    {
                        // Optionally stop the coroutine if needed
                    }
                }
                break;
        }
    ...
    private void CheckForPlayerDetection()
    {
        ...
        if (Physics.Raycast(transform.position, playerDirection, out hit, sightDistance))
        {
            if (hit.collider.CompareTag("Player"))
            {
                ...
                // The sanityManager will automatically start reducing sanity based on proximity
